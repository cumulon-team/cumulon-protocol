...
export class ChainConnector {
  ...
  protected BREAK_CHANGE_SPEC_VERSION = 0;
  protected NEED_CONVERT_ACCOUNT_ADDR = true;

  formatAccountAddr(addr) {
    if (this.NEED_CONVERT_ACCOUNT_ADDR) {
      return ChainUtils.ss58transform_simple(addr, this.network);
    }
    return addr;
  }
  ...
  async getCurrentRoundInfo(): Promise<any> {
    ...
    roundInfo["totalIssuance"] = this.formatWithDecimals(data.toString());
  ...
  api: ApiPromise;
  protected systemVersion: any;
  protected readonly logger: W3Logger;
  ...
  async init() {
    // TODO to introduce the round-robin algorithms for multi-endpoints
    ...
    //   {},
    // );
    this.wsProvider = new WsProvider(typeof wssEndpoint === 'string' ? wssEndpoint : wssEndpoint.url);
    ...
    this.wsProvider.on('connected', (val) => {
      this.logger.debug('connected:' + this.network.id);
    });
    this.wsProvider.on('disconnected', (val) => {
      this.logger.debug('disconnected:' + this.network.id);
    });
    this.wsProvider.on('error', (val) => {
      this.logger.error('error:' + this.network.id);
    });
  ...
  private isWaiting4Retrying = false;
  async checkReady() {
    let isReady = this.wsProvider.isConnected;
    let maxWait = 30;
    if (!isReady) {
      if (this.isWaiting4Retrying) {
        throw new Error(`${this.network.id} is re-connecting and has not ready yet`)
      }
      this.isWaiting4Retrying = true;
    }
