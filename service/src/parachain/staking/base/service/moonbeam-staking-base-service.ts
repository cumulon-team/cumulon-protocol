...
@Injectable()
export class MoonbeamStakingBaseService extends StakingBaseService {

  async getDelegatorRewardStatistic(request: DelegatorRewardStatisticRequest): Promise<DelegatorRewardStatisticResponse> {
    let resp: DelegatorRewardStatisticResponse = new DelegatorRewardStatisticResponse();
    ...
    if (record) {
      ...
      const { sum } = await DbManager.get(request).nrdhRepository
        .createQueryBuilder("d")
        .select("SUM(d.balance)", "sum")
        .where("d.account = :account", { account: request.delegatorAccount })
        .andWhere("d.realroundindex = :lastRoundIndex", { lastRoundIndex: lastRoundIndex })
        .getRawOne();
  ...
  async getCollatorRewardStatistic(request: CollatorRewardStatisticRequest): Promise<CollatorRewardStatisticResponse> {
    let resp: CollatorRewardStatisticResponse = new CollatorRewardStatisticResponse();
    ...
    if (record) {
      let lastRoundIndex = record.realroundindex;
      const { sum } = await DbManager.get(request).rhRepository
        .createQueryBuilder("d")
        .select("SUM(d.balance)", "sum")
  ...
  // async getDelegatorRewardHistory(request: DelegatorRewardHistoryRequest): Promise<DelegatorRewardHistoryResponse> {
  ...
  //   };
  // }
  // async getDelegatorActionHistory(request: DelegatorActionHistoryRequest): Promise<DelegatorActionHistoryResponse> {
  ...
  async getCollatorRewardHistory(request: CollatorRewardHistoryRequest): Promise<CollatorRewardHistoryResponse> {
  ...
  async getCollatorActionHistory(request: CollatorActionHistoryRequest): Promise<CollatorActionHistoryResponse> {
  ...
  async getCollatorProducedBlocks(request: CollatorProducedBlocksRequest): Promise<any> {
    let result: any = {};
    result.startRoundIndex = request.startRoundIndex;
    result.endRoundIndex = request.endRoundIndex;
    result.blocks = [];
...
//     return this.chainConnector.getLatestBlockNumber();
//   }
//   async getCurrentRoundInfo(): Promise<any> {
//     return this.chainConnector.getCurrentRoundInfo();
//   }
//   async getMaxNominatorsPerCollator(): Promise<any> {
//     return this.chainConnector.getMaxNominatorsPerCollator();
//   }
//   async getRealtimeCollatorCandidatePool(): Promise<any> {
//     return this.chainConnector.getRealtimeCollatorCandidatePool();
//   }
//   async getRealtimeCollatorState(collatorAccounts: string[]): Promise<any> {
//     return this.chainConnector.getRealtimeCollatorState(collatorAccounts);
//   }
//   async getScheduledExitQueue(): Promise<any> {
//     return this.chainConnector.getScheduledExitQueue();
//   }
//   async getSelectedCollators4CurrentRound(): Promise<any> {
//     return this.chainConnector.getSelectedCollators4CurrentRound();
//   }

  async getMaxCollatorsPerRound(request: StakingRequest): Promise<number> {

    let record = await DbManager.get(request).cnhRepository.findOne({
      order: {
        roundindex: "DESC"
      }
    })
    if (record) {
      ...
      let record = await DbManager.get(request).chainStateRepository.findOne();
      if (record) {
