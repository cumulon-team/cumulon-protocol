...
export class StakingBaseService {
  ...
  async getDelegatorRewardHistory(
    ...
    if (request.collator && request.collator.length) {
      where.collator = request.collator;
    }
    if (request.roundindex ) {
      where.realroundindex = request.roundindex;
    }
  ...
  async getDelegatorActionHistory(
    ...
    if (request.actionType) {
      where.actiontype = request.actionType
    }
