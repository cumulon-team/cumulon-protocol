<template>
  <a-spin style="width: 100%" :loading="loading">
    <div class="table-wrap">
      <el-table
        :data="
          formatTable.slice((pageIndex - 1) * pageSize, pageIndex * pageSize)
        "
      >
        ...
        <el-table-column prop="blocknumber" label="Block" min-width="120" />
...
import { timeMachineStakingHis } from "@/api/staking";

export default {
  props: ["selectCollator", "selectRound"],
  components: {
    IdentityWrap,
    IdentityIcon,
  },
  ...
  computed: {
    formatTable() {
      ...
      if (this.selectCollator) {
        arr = arr.filter((v) => v.collator == this.selectCollator);
      }
      if (this.selectRound) {
        arr = arr.filter((v) => v.roundindex == this.selectRound);
      }
      return arr;
    },
  },
